...
func TestEndpointsResolver_hasNamedPortBypassIssue(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "problematic: protocol mismatch - NP allows TCP/80 but named targetPort resolves to UDP/80",
			service: &corev1.Service{
				ObjectMeta: metav1.ObjectMeta{Name: "svc", Namespace: "ns"},
				Spec: corev1.ServiceSpec{
					ClusterIP: "10.0.0.1",
					Selector:  map[string]string{"app": "test"},
					Ports: []corev1.ServicePort{
						{Port: 80, TargetPort: intstr.FromString("dns"), Protocol: corev1.ProtocolTCP},
					},
				},
			},
			policyPorts: []networking.NetworkPolicyPort{
			...
			expected: true, // NP matches svc port TCP/80, named targetPort "dns" resolves to UDP/80 which != TCP/80
		},
		{
		...
		{
			name: "safe: multi-port service with NP matching only one port",
			service: &corev1.Service{
				ObjectMeta: metav1.ObjectMeta{Name: "svc", Namespace: "ns"},
				Spec: corev1.ServiceSpec{
					ClusterIP: "10.0.0.1",
					Selector:  map[string]string{"app": "test"},
					Ports: []corev1.ServicePort{
						{Name: "svc-port", Port: 6379, Protocol: corev1.ProtocolTCP, TargetPort: intstr.FromString("app-port")},
						{Name: "metrics", Port: 9101, Protocol: corev1.ProtocolTCP, TargetPort: intstr.FromString("metrics-port")},
					},
				},
			},
			policyPorts: []networking.NetworkPolicyPort{
			...
			pods: []corev1.Pod{
				{
					...
					Spec: corev1.PodSpec{
						Containers: []corev1.Container{
							{
								Ports: []corev1.ContainerPort{
									{Name: "probe", ContainerPort: 8888, Protocol: corev1.ProtocolTCP},
			...
			expected: false, // NP only matches svc port 6379, named targetPort "app-port" resolves to 6379 which is allowed. metrics-port (9101) is never checked.
		},
		{
			name: "problematic: nil protocol in NP defaults to TCP and detects bypass",
			service: &corev1.Service{
				ObjectMeta: metav1.ObjectMeta{Name: "svc", Namespace: "ns"},
				Spec: corev1.ServiceSpec{
					ClusterIP: "10.0.0.1",
					Selector:  map[string]string{"app": "test"},
					Ports: []corev1.ServicePort{
						// Service port 80, named targetPort "http", protocol omitted (defaults to TCP)
						{Name: "http", Port: 80, TargetPort: intstr.FromString("http")},
					},
				},
			},
			policyPorts: []networking.NetworkPolicyPort{
				// NP allows port 80 with nil Protocol (defaults to TCP)
				{Port: &intOrStrPort80},
			},
			pods: []corev1.Pod{
				{
			...
			expected: true, // Without nil-to-TCP defaulting this would incorrectly return false (safe)
		},
	}
